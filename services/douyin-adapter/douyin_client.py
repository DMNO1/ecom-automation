...
from models import (
    DouyinConfig, ProductInfo, OrderInfo, InventoryUpdate,
    AftersaleInfo, LogisticsInfo, ApiResponse, PaginatedResponse
)
...
class DouyinClient:
    ...
    API_ENDPOINTS = {
        ...
        # 售后API
        "aftersale_list": "/api/aftersale/list",
        
        # 物流API
        "logistics_send": "/api/logistics/send",
    }
    
    def __init__(self, auth_manager: DouyinAuthManager):
    ...
    async def get_aftersale_list(
        ...
        result = await self._request("GET", self.API_ENDPOINTS["aftersale_list"], params=params)
...
# 全局客户端实例
_client: Optional[DouyinClient] = None
...
def get_douyin_client() -> DouyinClient:
    ...
    if _client is None:
        ...
        _client = DouyinClient(auth_manager)
    return _client


def init_douyin_client(auth_manager: DouyinAuthManager) -> DouyinClient:
    """初始化抖店客户端"""
    global _client
    _client = DouyinClient(auth_manager)
    return _client
