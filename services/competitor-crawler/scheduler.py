...
class TaskScheduler:
    ...
    async def _execute_price_check(self, task: ScheduledTask):
        ...
        async with self._locks[task_id]:
            try:
                logger.info(
                    f"Executing price check: {task.platform.value}/{task.product_id}"
                )
                    ...
                    if last_snapshot and last_snapshot.price != result.data.price:
                        ...
                        if abs(change_percent) > 10:
                            await self._trigger_price_alert(
                                task, last_snapshot.price, result.data.price
                            )
    ...
    async def _execute_title_check(self, task: ScheduledTask):
        ...
        async with self._locks[task_id]:
            try:
                logger.info(
                    f"Executing title check: {task.platform.value}/{task.product_id}"
                )
                ...
                if result.success and result.data:
                    ...
                    if last_snapshot and last_snapshot.title != result.data.title:
                        ...
                        await self._trigger_title_alert(
                            task, last_snapshot.title, result.data.title
                        )
    ...
    async def _execute_full_check(self, task: ScheduledTask):
        ...
        async with self._locks[task_id]:
            try:
                logger.info(
                    f"Executing full check: {task.platform.value}/{task.product_id}"
                )
                ...
                if result.success and result.data:
                    ...
                    if promotions:
                        logger.info(
                            f"Active promotions for {task.product_id}: {len(promotions)}"
                        )
    ...
    async def _execute_promotion_monitor(self, task: ScheduledTask):
        ...
        async with self._locks[task_id]:
            try:
                logger.info(
                    f"Executing promotion monitor: {task.platform.value}/{task.product_id}"
                )
                ...
                if result.success and result.data:
                    ...
                    if promotions:
                        ...
                        for promo in promotions:
                            logger.info(
                                f"  - {promo.promotion_type}: {promo.promotion_detail}"
                            )
    ...
    async def send_notification(self, message: str):
        """发送通知"""
        webhook_enabled = (
            os.environ.get("NOTIFICATION_WEBHOOK_ENABLED", "false").lower() == "true"
        )
    ...
    async def _trigger_price_alert(
        self, task: ScheduledTask, old_price: float, new_price: float
    ):
        """触发价格告警"""
    ...
    async def _trigger_title_alert(
        self, task: ScheduledTask, old_title: str, new_title: str
    ):
        """触发标题变化告警"""
        alert_message = (
            f"标题变化告警！\n"
...
# 便捷函数
async def create_price_check_task(
    platform: Platform, product_id: str, url: str, interval_minutes: int = 15
) -> str:
    """创建价格检查任务"""
    scheduler = await get_scheduler()
...
async def create_title_check_task(
    platform: Platform, product_id: str, url: str, interval_minutes: int = 30
) -> str:
    """创建标题检查任务"""
    scheduler = await get_scheduler()
...
async def create_full_check_task(
    platform: Platform, product_id: str, url: str, interval_minutes: int = 60
) -> str:
    """创建完整检查任务"""
    scheduler = await get_scheduler()
